...
function Practitioner() {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/addpractitioner`, requestOptions)
      ...
      .then((result) => {
        if (result === "user found") {
          message.warning("Practitioner Already in List");
        } else {
  ...
  const handleInputChange = (e) => {
    ...
    const filteredSuggestions = practitioner.filter(
      (item) =>
  ...
  const [selectedButton, setSelectedButton] = useState(urlTest);
  ...
  const handleUpdateReportType = async (e, value) => {
    e.preventDefault();

    // Determine the selected button value
    const selectedValue = value === 0 ? urlTest : urlSupply;

    // Set the state for selectedButton
    setSelectedButton(selectedValue);

    // Display loading message
    const hide = message.loading("Updating report type...", 0);

    try {
      // Prepare the request data
      const requestData = {
        _id: userinfo._id, // Pass the practitioner's ID here
        reportType: selectedValue, // Use the determined value directly
      };

      // Send the POST request to update the report type
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/updatereporttype`,
        {
          method: "PUT",
          headers: {
      ...
      if (response.ok) {
        ...
        message.success("Report type updated successfully");
      } else {
        // Handle error response
        message.error("Failed to update report type");
      }
    } catch (error) {
      console.error("Error updating report type:", error);
      message.error("An error occurred while updating the report type");
    } finally {
      hide(); // Hide the loading message
      setpopupdetails(false); // Close the popup
      var myHeaders = new Headers();
      myHeaders.append("Content-Type", "application/x-www-form-urlencoded");
                  ...
                  <div className="popup">
                    ...
                    >
                      ...
                      <input
                        ...
                        style={{ borderRadius: "5px" }}
                        onChange={(e) =>
                          setpractitionerpassword(e.target.value)
                        }
                        type="password"
                        name="nedfvw"
                        value={practitionerpassword}
                        required
                        placeholder="Password for Practitioner"
                      />
                    ...
                    <div className="profilemainbody" style={{ width: "95%" }}>
                      ...
                      <div className="uperprofile">
                        ...
                        <div className="profileabot">
                          ...
                          <div>
                            <h3 style={{ width: "200px" }}>Registered On : </h3>{" "}
                            <p style={{ margin: "0" }}>
                              {" "}
                              {userinfo.timestamp ? (
                                <>{userinfo.timestamp}</>
                          ...
                          <div>
                            <h3 style={{ width: "200px" }}>Action </h3>
                            <a
                              onClick={(e) => {
                                e.preventDefault();
                                setpopup22(true);
                      ...
                      <div className="profileinfo">
                        <div className="infoboxinside">
                          {userinfo.assignedkits !== undefined && (
                            <>
                              {" "}
                              <h2>{userinfo.assignedkits.length}</h2>
                            </>
                          )}
                          {userinfo.assignedkits === undefined && (
                            <>
                        ...
                        <div className="infoboxinside">
                          ...
                          <h2>{Conuttttttt2()}</h2>
                          <p style={{ margin: "0" }}>Pending results</p>
                        </div>
                      </div>
                      <div
                        className=""
                      ...
                      >
                        <div className="" style={{color:"#19b0e6"}}>
                        <h3>Select Report Type : </h3>
                        </div>
                        <button
                          onClick={(e) => {
                            handleUpdateReportType(e, 0);
                          }}
                          style={{
                            padding: "5px 15px",
                            borderRadius: "15px",
                      ...
                      >
                        ...
                        <a
                          onClick={async (e) => {
                            ...
                            await fetch(
                              `${process.env.REACT_APP_API_URL}/dltpractitioner`,
                              requestOptions
                            )
                              .then((response) => response.json())
                              .then((result) => {
                            ...
                            {popup2 === index && (
                              <>
                                ...
                                <div className="popup">
                                  <div className="header">
                                    <h4>
                                      Assigned {item.assignedkits.length} Kits
                                      to {item.name}
                                    </h4>
                                  </div>
